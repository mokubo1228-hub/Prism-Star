...
- User の方向性を **Codex-ready な handoff（実装指示）** に落とす。
...
- **実装は原則 Codex に handoff で渡す。** Claude Code がコードを直接変更するのは、
...
- Codex への橋渡しは常に **Claude Code の handoff docs** で、
  **人間がそれを Codex に渡す**。
...
- `docs/design/highlights.md`: **ポートフォリオ読みどころ**（こだわり・横断する判断軸・技術スタックと選定理由）。ADR/spec への索引。
- `docs/spec/spec.md`: **技術仕様書**（v1 現状：技術スタック・アーキテクチャ・データモデル・API・要件・セキュリティ）。
