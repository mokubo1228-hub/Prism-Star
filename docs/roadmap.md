...
## 次にやる（優先トラック）
機能の核は出揃ったので、ここからは「プラットフォームとしての堅牢性・基本機能の穴埋め」を優先する。

### ① セキュリティ強化（CSRF / セッション Cookie）⬜
マルチユーザーで投稿/削除/スター/取り込み/認証があるのに CSRF 防御が無く、セッション Cookie の属性も未設定。セッション開始を共通化して **SameSite=Lax＋HttpOnly＋Secure** を付与（cross-site CSRF をほぼ封殺）し、さらに状態変更系に **CSRF トークン**（中央化した検証＋fetch 共通付与）。ADR・handoff は着手時に作成。

### ② アカウント設定（表示名・パスワード変更）⬜
ログイン中に表示名・パスワードを変更できない（変更可は github_username のみ）。マイページに「アカウント設定」を追加し、`auth.php`/`users.php` に本人限定の更新を足す（パスワード変更は現行パスワード確認つき）。基本機能の穴埋め。

### ③ ページネーション ⬜
一覧・検索が全件返し。`LIMIT/OFFSET` か「もっと見る」で段階読み込み。seed 規模では動くが「プラットフォーム」としては要る。

## 今後の発展（展望・後回し）⬜
- タグ予測（`#` 入力中に既存タグを候補表示。`type=tags` エンドポイント＋`<datalist>`）。
- ブランド表現（虹色テーマ）の作り込み・レスポンシブ細部（デザインは終盤にまとめる＝[進め方の原則]）。
- username / slug（公開 URL）の導入（登録の同時実行堅牢化は double opt-in で吸収済み）。
