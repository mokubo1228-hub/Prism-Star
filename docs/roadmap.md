...
## 次の本命 — GitHub 取り込みの永続化 ⬜（handoff・ADR 済／GO 待ち）
看板の「全員が発信者 ＋ GitHub 取り込み」のうち、**取り込みがまだ表示レイヤーだけ**（`github.php` が GitHub API を叩いてプロフィールにリポジトリを並べるのみ・[ADR-016](decisions.md)）。リポジトリを **PrismStar の作品として `gallery` に保存**し、⭐・検索・おすすめ・プロフィールに乗せる（[ADR-024](decisions.md)）。`gallery` に `source`／`source_url` を足し、サーバ側で GitHub を再取得して保存値を確定（client を信用しない）、`(user_id, source_url)` で de-dupe（再取り込みはリフレッシュ）、fork 除外、取り込みはマイページ操作、サムネは OG カード画像。handoff：`docs/github-import-handoff.md`（Codex へ渡せる状態）。
