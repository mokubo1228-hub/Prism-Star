...
- **第3段 ⬜（[ADR-034](decisions.md)・設計のみ・handoff 未）**：**ユーザーネーム `@username`**（一意ハンドル）＝`users.username`（UNIQUE）追加・登録時に自動採番＋後で編集・`profile.php?u=<username>` ルックアップ（`?id=` も維持・pretty URL は後回し）。
> 第3段（@username）は schema 追加・登録フロー波及を含むため、別 handoff で着手する。

## 作品詳細の「戻る」を直前の画面に返す（[ADR-035](decisions.md)）⬜（handoff 済）
