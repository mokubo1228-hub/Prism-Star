...
## 検索結果を番号付きページングへ（総件数・表示数切替）（[ADR-044](decisions.md)）✅ 実装・review 済（コミットは区切りで）
[ADR-029](decisions.md) は検索を「もっと見る」にし番号ページ／`COUNT` を「本規模では過剰」と却下したが、**検証用 seed の増量で対象件数が増え検索が網羅ブラウズの実面になった**ため前提を見直す。検索（**works/users 両方**）を**番号付きページング**（前へ/次へ＋ページ番号・`page`/`per_page` を URL 同期・リロード/共有で復元）に置き換え、**総件数 `total`**（works＝同一 WHERE で `COUNT(DISTINCT g.id)`／users＝一致ユーザー数）と**表示件数切替 `10/30/50`（既定30・allowlist）**を入れる。**表示件数切替時はアンカー優先**＝先頭へ戻さず直前の閲覧位置を含むページへ移動する（半端ページ着地は許容。API/URL の `page=1` は常に検索順の先頭から最大件数）。視覚は **検索結果のみ5列・カード小さめ（情報量優先）／おすすめ・人気レーンは横スクロール前提でカード拡大**＝[ADR-029](decisions.md) の「トップ＝キュレーション／検索＝網羅」を表示にも延長（お気に入り/プロフィールは4列据え置き）。トップの2軸ランキングは維持し、検証 seed の追加分は `created_at` を古めにしてトップ新着レーンを埋めない。自分除外・公開のみ・非公開非開示・並び順は不変、スキーマ変更なし。handoff：`docs/search-results-pagination-handoff.md`（仕様ドラフト：`docs/search-results-pagination-spec.md`）。

## 大量ダミーデータ：seed を ~500 公開作品・~40 ユーザーに（[ADR-044](decisions.md) の seed増量を拡張）⬜ handoff 済
[ADR-044](decisions.md) の検証用 seed増量（+30）は「ページングが動くか」の最小確認まで。検索の総件数・ページャ・「賑わったプラットフォーム」を実感できるよう、`src/seed.php` を **~500 公開作品 / ~40 ユーザー** の**プログラム生成**（決定的・乱数なし・冪等）に拡張する。テスト臭い「検索検証 …」ラベルは廃し、語彙の組み合わせで**自然な作品名/タグ**に（`src/seed.php`・デモとも面接官に見える前提＝[doc-visibility-principle]）。**不変条件**：既存の作品1〜24は"顔"として維持・バルクは `created_at` 古めでトップ新着 top5（[ADR-029](decisions.md)）を埋めない・star 傾斜で人気ランキングは維持・公開/非公開ミックス・自分除外・冪等（2回流して不変）。スキーマ・API・並び順は不変。実装は Codex。handoff：`docs/seed-dummy-data-handoff.md`。
