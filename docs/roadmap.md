...
## マイページ分割（作品管理 ／ 設定）⬜（[ADR-030](decisions.md)・handoff 済）
...
- **第1段（[ADR-031](decisions.md)＋[ADR-032](decisions.md)・handoff 済）**：個人系（作品管理／お気に入り／プロフィール／設定）を**ハンバーガーに並列集約**し「マイページ」総称を廃止（ラベルは「作品管理」へ）。**お気に入りページ**を新設（`gallery.php ?starred=1` ＝自分がスターした公開/自作作品。片側だけだったスター機能の穴埋め）。handoff：`docs/personal-area-handoff.md`。
- **第2段（[ADR-033](decisions.md)・設計のみ）**：プロフィール拡充①＝`users.bio`（自己紹介）追加・**表示名を設定→プロフィールへ移設**（設定はパスワード＋GitHub username に純化）・profile に owner 編集。
- **第3段（[ADR-034](decisions.md)・設計のみ）**：**ユーザーネーム `@username`**（一意ハンドル）＝`users.username`（UNIQUE）追加・登録時に自動採番＋後で編集・`profile.php?u=<username>` ルックアップ（`?id=` も維持・pretty URL は後回し）。
> 第2/3段は schema 追加・登録フロー波及を含むため、第1段が落ちてから handoff 化する。
