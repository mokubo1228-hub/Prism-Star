...
- 次バージョンの要件・動線：`docs/spec/requirements-v2.md`
- 設計判断とその理由：`docs/adr/decisions.md`
...
GitHub の star とサイトのお気に入りが地続きになり、GitHub 連携が"取ってつけ"でなくコンセプトの中心に溶ける（[ADR-009](../adr/decisions.md)）。

### ターゲット
- 非エンジニア … 画像 + 説明文を発信。
- エンジニア … GitHub リポジトリを発信。

命名理由は [ADR-006](../adr/decisions.md)、プラットフォーム化は [ADR-008](../adr/decisions.md)。
...
| 外部連携 | GitHub REST API | **サーバ側 PHP から呼び、token は env**（[ADR-003](../adr/decisions.md)） |

技術選定の理由は [ADR-002](../adr/decisions.md)（PHP 維持）/ [ADR-003](../adr/decisions.md)（バックエンド経由）。
...
docs/              adr / spec / design / assets / roadmap / roles（公開の設計doc）
```
...
**tags** / **gallery_tags**：タグを正規化テーブルで多対多に保持（[ADR-017](../adr/decisions.md)）
...
**pending_registrations**：id / email / token_hash / expires_at / created_at（登録 double opt-in。[ADR-018](../adr/decisions.md)）
**password_resets**：id / user_id / token_hash / expires_at / created_at（パスワード再設定。[ADR-021](../adr/decisions.md)）
...
| メソッド / エンドポイント | 機能 | 認証 |
|---|---|---|
| `GET /api/gallery.php` / `?id=N` | 一覧 / 1件取得（おすすめは自分の作品を除外・[ADR-027](../adr/decisions.md)） | 一部要 |
...
| `GET /api/search.php?q=&type=works｜users&tag=` | 作品 / ユーザー / タグ検索（[ADR-025](../adr/decisions.md)） | 要 |
...
| `GET /api/github.php?user=` | GitHub リポジトリ取得（token はサーバ側・[ADR-003](../adr/decisions.md)） | 要 |
| `/api/auth.php` | ログイン / ログアウト / `?action=status` / 新規登録〔double opt-in・[ADR-018](../adr/decisions.md)〕/ メール確認 / パスワード再設定 | — |
...
- **検索の番号付きページング**（総件数・表示件数 10/30/50 切替・URL で状態復元・[ADR-044](../adr/decisions.md)）/ 共通ナビ・ハンバーガー・レスポンシブ。
