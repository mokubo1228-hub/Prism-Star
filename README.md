...
- **発見** — 検索（作品 / ユーザー / #タグ・「もっと見る」で段階読み込み）と、トップの **新着・人気の2軸ランキング**（各トップ5のキュレーション）。
...
- **発見系を役割で分ける情報設計** — 検索＝網羅（段階読み込み）／トップ＝キュレーション（新着・人気の2軸）。1画面に詰め込まない（ADR-029）。
...
※ seed では他にも複数ユーザー（Aoi / Ren / Mio・全員 `password123`）を投入し、所有モデルや人気ランキングが実データで見えるようにしている。
